...
class baseModel():
    def __init__(self, net, config, all_random_labels, crossValidation=False, resultSavePath=None, vis=None):
        ...
        self.all_random_labels = all_random_labels
    ...
    def train(self, trainDataset, valDataset, lossFunc, optimizer, all_random_labels, scheduler=None):
        self.lossFunc = lossFunc
        self.optimizer = optimizer
        self.scheduler = scheduler

        trainResults = self._train(trainDataset, valDataset, all_random_labels)

        if self.resultSavePath is not None:
            with open(os.path.join(self.resultSavePath, 'TrainResults.txt'), 'w') as fp:
                for key, value in trainResults.items():
    ...
    def test(self, testDataset, classes, all_random_labels):
        preds, trues = self.predict(testDataset, all_random_labels)
    ...
    def _train(self, trainDataset, valDataset, all_random_labels):
        valResults = []
        trainLoss = []
        ...
        while not doStop:
            if not earlyStopReached:
                lossOneEpoch = self._trainOneEpoch(trainDataset, all_random_labels)
                trainLoss.append(lossOneEpoch)
                if self.scheduler is not None:
                    self.scheduler.step(lossOneEpoch)

                pred, act = self.predict(valDataset, all_random_labels)
    ...
    def _trainOneEpoch(self, trainDataset, all_random_labels):
        self.net.train()
        ...
        with torch.enable_grad():
            for data, label in dataLoader:
                ...
                label1 = label.cpu()
                all_random_labels.append(label1.detach().numpy())
    ...
    def predict(self, dataset, all_random_labels):
        predicted = []
        actual = []
